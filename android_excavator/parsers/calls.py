"""Call history from calllog.db / contacts databases (read-only)."""
from __future__ import annotations

from pathlib import Path
from typing import List

from android_excavator.core.models import ArtifactRecord, Provenance, Quality
from android_excavator.core.source import FolderDump
from android_excavator.core.sqlite_ro import open_evidence_ro
from android_excavator.parsers.base import Parser, ParserInfo


class CallLogParser(Parser):
    info = ParserInfo(
        parser_id="calls",
        label="Phone calls",
        description="Reads call history (number, time, duration, type).",
        category="communications",
    )

    def parse(self, dump: FolderDump, *, limit: int = 50_000) -> List[ArtifactRecord]:
        records: List[ArtifactRecord] = []
        paths = dump.find_by_name("calllog.db")
        paths += dump.find_by_name("calls.db")
        paths += dump.find_by_name("contacts2.db")  # older Android stores calls here
        paths += dump.find_path_contains("com.android.providers.contacts")
        paths = list({p.resolve() for p in paths if p.suffix.lower() == ".db"})

        for db_path in paths:
            try:
                records.extend(self._parse_db(dump, db_path, limit=limit - len(records)))
            except Exception:
                continue
            if len(records) >= limit:
                break
        return records[:limit]

    def _parse_db(self, dump: FolderDump, db_path: Path, *, limit: int) -> List[ArtifactRecord]:
        out: List[ArtifactRecord] = []
        logical = dump.logical_path(db_path)
        try:
            digest = dump.sha256_file(db_path)
        except Exception:
            digest = ""

        conn = open_evidence_ro(db_path)
        try:
            tables = {r[0] for r in conn.execute(
                "SELECT name FROM sqlite_master WHERE type='table'"
            )}
            table = next((t for t in ("calls", "call_log", "calllog") if t in tables), None)
            if not table:
                return out
            cols = {r[1] for r in conn.execute(f'PRAGMA table_info("{table}")')}
            num_col = next((c for c in ("number", "phone_number", "normalized_number") if c in cols), None)
            date_col = next((c for c in ("date", "datetime", "time") if c in cols), None)
            dur_col = "duration" if "duration" in cols else None
            type_col = "type" if "type" in cols else None
            if not date_col:
                return out

            select = ["ROWID", f'"{date_col}"']
            if num_col:
                select.append(f'"{num_col}"')
            if dur_col:
                select.append(f'"{dur_col}"')
            if type_col:
                select.append(f'"{type_col}"')
            sql = f'SELECT {", ".join(select)} FROM "{table}" ORDER BY "{date_col}" DESC LIMIT ?'
            for row in conn.execute(sql, (limit,)):
                out.append(
                    ArtifactRecord(
                        artifact_type="android.call",
                        record={
                            "rowid": row["ROWID"],
                            "number": row[num_col] if num_col else None,
                            "date": row[date_col],
                            "duration": row[dur_col] if dur_col else None,
                            "type": row[type_col] if type_col else None,
                        },
                        provenance=Provenance(
                            source_logical_path=logical,
                            source_real_path=str(db_path),
                            source_sha256=digest,
                        ),
                        quality=Quality(confidence=0.9),
                    )
                )
        finally:
            conn.close()
        return out
