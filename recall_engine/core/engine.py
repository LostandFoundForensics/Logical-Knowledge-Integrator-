...
from recall_engine.parsers import all_parsers
from recall_engine.parsers.base import Parser
...
def scan_backup(
    ...
    *,
    ...
    with ManifestIndex(Path(backup_path)) as manifest:
        ...
        for p in parsers:
            try:
                hits.extend(p.parse(manifest, limit=limit_per_parser))
            except Exception:
                continue
        return hits
